...
Either way, the setup page checks the account at the end of Step 3 and shows what Spotify answered, including what is currently playing. If a widget still shows "Nothing playing" with a reason underneath, open the setup page (`setup-spotify.bat` / `.sh`), load (or paste) that `settings.txt` into the **Troubleshooting** box, and test it: the page does exactly what the widget does and prints Spotify's answers. Testing uses the token up, so save the updated file the test hands back; the Save button writes it straight into the widget folder, and with the helper running that happens by itself when the file tested was the folder's own (someone else's file never replaces yours).
