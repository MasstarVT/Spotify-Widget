...
- Each poll is one request, so `poll_interval=500` means 120 requests a minute from a single widget. The default `2000` (30 a minute) is plenty for an overlay: a track change still shows within about two seconds.
- Widgets in the same OBS share their polling. Only one of them asks Spotify and the others reuse its answer, so extra scenes or widget styles do not multiply the request rate.
- Browsers are not allowed to read Spotify's `Retry-After` header, so after a 429 the widget waits 30 seconds, then 1, 2, 4, 8 and at most 10 minutes between attempts until Spotify answers normally again. Every widget in that OBS waits together, and with `source=auto` Snip is used meanwhile. If nothing is on screen the widget shows "Spotify rate limited, retrying in …".
- The Troubleshooting test on `spotify-setup.html` uses three requests.
...
| `poll_interval` | How often to check for track changes, in ms (default `2000`, minimum `500`). Lower values spend Spotify's request allowance faster; see [Rate limits](#rate-limits) |
